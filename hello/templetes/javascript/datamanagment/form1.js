...
$("#submit").click(function(){    //onsubmit   
    ...
    localStorage.setItem(id,JSON.stringify(tempRecord));
...
function checkName(){       
    ...
    if(name=="" || /^[a-zA-Z]*\s[a-zA-Z]*\s[a-zA-Z]*$|[a-zA-Z]*\s[a-zA-Z]*$|[a-zA-Z]*$/.test(name)==false){
...
function checkAge(){ 
    ...
    if(/^[0-9]{2}$/.test(age)==false){
        ...
        window.alert("Wrong age Format Age Must be >18 and <60");
        validatedAge=false;
        return;
...
function updateRecord(name,age,password,idx){
    ...
    localStorage.setItem(idx,JSON.stringify(records[indexToUpdate]));
    idToUpdate="";
    printRecords();
